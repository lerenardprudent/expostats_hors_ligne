...
sequential.plot.frac <-function( gm , gsd , frac , c.oel , data.simply.imputed,
                                 pal_col = TRUE,
  ...
  data <-data.frame(seq=1:240,
                    x=exp(rnorm(240,mu,sigma)))
  ...
  graph2 <- ggplot(data=data, aes(y=x,x=seq))+
    ...
    annotate("text", x = 130, y = c.oel*1.65, colour = "black",
             ...
             size=6)+
    ylim(c(0,pmax(2*c.oel,max(data.simply.imputed$imputed$data$xfin))))+
    # theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+ #suppression to add scale_x_continuous
    ...
    scale_x_continuous(breaks = seq(1, max(data$seq), length.out = 12), labels = c(1:12))
  
  
  #######observed points (50 or less)
  
  #observed less than 50
  
  n.obs <- length(data.simply.imputed$imputed$data$xfin)
  
  if (n.obs<=50)  {
    
    data.to.plot <- data.simply.imputed$imputed$data$xfin
    data.to.plot.notcensored <- data.simply.imputed$notcensored
  
  }
  
  if (n.obs>50)  {
    
    data.to.plot <- data.simply.imputed$imputed$data$xfin[ sample( 1:n.obs , size = 50 ) ]
    data.to.plot.notcensored <- data.simply.imputed$notcensored[ sample( 1:n.obs , size = 50 ) ]
  ...
  restrict <- data.to.plot.notcensored
  
  data.observed <-data.frame(x=sample(1:240 , size = length(data.to.plot[restrict])),
                             y=data.to.plot [restrict])
  
  restrict <- !data.to.plot.notcensored
  
  data.censored <-data.frame(x=sample(1:240 , size = length(data.to.plot[restrict])),
                             y=data.to.plot[restrict])
  
  ## graphs
  
  graph2 <- graph2+
    geom_point(data=data.observed,aes(x=x,y=y),colour = vec_color[1], fill = vec_fill[1], size= 4, shape =  ifelse(pal_col == TRUE, 20, 21), stroke = 1.5)+
    geom_point(data=data.censored,aes(x=x,y=y),colour = vec_color[2], fill = vec_fill[2], size=3, shape =  ifelse(pal_col == TRUE, 17, 24), stroke = 1.5)+
    
    theme(legend.position="none") + theme(axis.line = element_line(size = 3, colour = "grey80")) +
    theme(axis.ticks = element_line(size = 2))
  graph2
  
  # ggsave("Seq_plot_frac_fig1_BW.tiff", graph2, dpi = 600)
  # ggsave("Seq_plot_frac_fig1_BW.pdf", graph2, dpi = 600)
  # ggsave("Seq_plot_frac_fig1_BW.png", graph2, dpi = 600)
...
sequential.plot.perc <-function(gm , gsd , perc , target_perc , c.oel , data.simply.imputed,
                                pal_col = TRUE,
                                seqplot.1="Concentration",
                                seqplot.3="OEL",
  ...
  graph2 <- ggplot(data=data, aes(y=x,x=seq))+
    ...
    geom_point(color= "gray15",size=1.8, shape = 16)+
    ylim(c(0,pmax(1.2*perc,1.2*c.oel,max(data.simply.imputed$imputed$data$xfin))))+
...
sequential.plot.am <-function(gm , gsd , am , c.oel , data.simply.imputed,
                              pal_col = TRUE,
                              seqplot.1="Concentration",
                              seqplot.3="OEL",
  ...
  graph2 <- ggplot(data=data, aes(y=x,x=seq))+
    ...
    geom_point(color= "gray15",size=1.8, shape = 16) +
    ylim(c(0,pmax(1.5*c.oel,1.5*am,max(data.simply.imputed$imputed$data$xfin))))+
    # theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+
    theme(axis.text.y=element_text(size=14),axis.title.y=element_text(size=16,vjust=+0.55))+
    theme(axis.text.x = element_text(size = 14),axis.title.x = element_text(size = 16,vjust=-1.55))+
    labs(x=seqplot.6, y=seqplot.1) +
